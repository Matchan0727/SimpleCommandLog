...
import static jp.simplespace.simplecommandlog.bungee.BSimpleCommandLog.noPermission;

public class BTypeLog extends Command implements Listener {
    public BTypeLog(){
        super("bstl","scl.command.stl","btl","btypelog");
    }
    @Override
        ...
        if(!(sender instanceof ProxiedPlayer)){
            ...
            if(p.hasPermission("scl.command.stl")){
                ...
                if (list.contains(p.getUniqueId().toString())){
                    ...
                    p.sendMessage(new TextComponent(prefix + ChatColor.GRAY + "入力ログ表示を無効にしました。"));
                }
                else {
                    list.add(p.getUniqueId().toString());
                    p.sendMessage(new TextComponent(prefix + ChatColor.GRAY + "入力ログ表示を有効にしました。"));
                }
    ...
    public void onType(TabCompleteEvent event){
        ...
        for(String puuid : list){
            ...
            if(p!=null) p.sendMessage(new TextComponent(ChatColor.GRAY + "[TL] "+sender.getServer().getInfo().getName()+"@" + sender.getName() + " " + event.getCursor()));
        }
    }
}
